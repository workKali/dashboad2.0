...
export const badgeStyles = cva(['inline-flex items-center gap-1 rounded-[7px] px-2.5 py-[3px] text-base whitespace-nowrap'], {
	variants: {
		variant: {
			warning: 'bg-opacity-amber-light text-warning',
			harvest: 'bg-amber-100 text-amber-700',
			neutral: 'bg-gray-50 text-muted font-semibold',
...
export const Badge = ({ children, variant, weight, className }: BadgeProps) => {
	return <span className={clsx(badgeStyles({ variant, weight }), className)}>{children}</span>;
};
