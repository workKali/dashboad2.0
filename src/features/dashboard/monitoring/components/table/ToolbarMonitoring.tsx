...
interface ToolbarMonitoringProps {
...
export const ToolbarMonitoring = ({ table }: ToolbarMonitoringProps) => {
	...
	const handleSortChange = (value: string) => {
		setSortBy(value);
		// Map sort options to column sorting
		const sortMap: Record<string, { id: string; desc: boolean }> = {
			riesgo_desc: { id: 'risk', desc: true },
			ndvi_asc: { id: 'ndvi', desc: false },
			ndvi_desc: { id: 'ndvi', desc: true },
			cosecha_desc: { id: 'harvestProbability', desc: true },
			ha_desc: { id: 'hectares', desc: true },
			finca_asc: { id: 'farm', desc: false },
		};

		const sortConfig = sortMap[value];
		if (sortConfig) {
			table.setSorting([sortConfig]);
		}
	};
	...
	return (
		<div className="flex gap-2 items-center flex-wrap mb-2.5 bg-gray-50 border border-gray-200 rounded-xl p-2 px-3">
			<SearchInput placeholder="Buscar finca, productor, región…" value={searchValue} onChange={handleSearchChange} />

			<FilterSelect label="Riesgo" options={riskOptions} value={riskFilter} onChange={setRiskFilter} />

			<FilterSelect label="Actividad" options={activityOptions} value={activityFilter} onChange={setActivityFilter} />

			<FilterSelect label="Región" options={regionOptions} value={regionFilter} onChange={setRegionFilter} />

			<FilterSelect label="Orden" options={sortOptions} value={sortBy} onChange={handleSortChange} />

			<span className="text-base text-secondary ml-auto whitespace-nowrap">
				{filteredRowCount} de {totalRowCount} fincas
			</span>
		</div>
	);
};
